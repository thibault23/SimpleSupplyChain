...
contract SupplyChain {
  ...
  */
  ...
  struct Item {
     ...
     State state; // [me] pay attention to how the state is declared, there was an error when I pass it as an array
  ...
  /* Create 4 events with the same name as each possible State (see above)
   ...
   // careful about event naming here and in the testing js file
...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier checkValue(uint _sku) {
    ...
    uint amountToRefund = msg.value - _price; // [me] does msg.value here still retain the initial value?
  ...
  modifier forSale (uint _sku) {
      ...
      bytes memory nameConverted = bytes(items[_sku].name); //[me] initially forgot the keyword memory

      require(nameConverted.length != 0);
...

  {
      ...
      //we can't transfer the whole amount else we would be transferring more money than expected
      items[sku].seller.transfer(items[sku].price); // unsafe to put it at the top (proceed with the state updates first) -> example of the DAO hack
